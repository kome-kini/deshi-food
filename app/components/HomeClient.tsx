...
const slides = [
  ...
  { image: "/media/rice.jpg", kicker: "চাল ও শস্য • নেত্রকোণা", title: "কালিজিরা সুগন্ধি চাল, ব্যাচ-ট্রেস সহ", cta: "চাল ঘুরে দেখুন", href: "/catalog?category=চাল%20ও%20শস্য" },
  { image: "/media/spices.jpg", kicker: "মসলা • বাগেরহাট থেকে যশোর", title: "অঞ্চলের আসল স্বাদ, দায়িত্বশীল দাবিসহ", cta: "মসলা দেখুন", href: "/catalog?category=মসলা" },
];
  ...
  return (
    <main className="amz-home">
      ...
      <section className="amz-quad section-shell" aria-label="Quick shopping shortcuts">
        <div className="amz-quad-card"><h2>আজকের ডিল</h2><div className="amz-quad-img"><Image src="/media/seasonings.jpg" alt="Today's deals" fill sizes="280px" /></div><Link href="/catalog?sort=featured">ডিল দেখুন</Link></div>
        <div className="amz-quad-card"><h2>বেস্ট সেলার</h2><div className="amz-quad-img"><Image src="/media/rice.jpg" alt="Best sellers" fill sizes="280px" /></div><Link href="/catalog">সব দেখুন</Link></div>
        <div className="amz-quad-card"><h2>Trace-ready প্যান্ট্রি</h2><div className="amz-quad-img"><Image src="/media/spices.jpg" alt="Trace ready products" fill sizes="280px" /></div><Link href="/catalog?trace=1">উৎস দেখুন</Link></div>
        <div className="amz-quad-card"><h2>মধু ও মিষ্টি</h2><div className="amz-quad-img"><Image src="/media/honey.jpg" alt="Honey and sweets" fill sizes="280px" /></div><Link href="/catalog?category=মধু%20ও%20মিষ্টি">ঘুরে দেখুন</Link></div>
      </section>

      <section className="amz-strip"><header><h2>ক্যাটাগরি অনুযায়ী কিনুন</h2><Link href="/catalog">সব ক্যাটাগরি</Link></header><div className="amz-cat-row">{categories.slice(0, 8).map((category) => <Link key={category.name} href={`/catalog?category=${encodeURIComponent(category.name)}`} className="amz-cat-tile"><span className="amz-cat-img"><Image src={categoryImage(category.name)} alt={category.en} fill sizes="170px" /></span><span>{category.name}</span></Link>)}</div></section>
