import { ensureRuntimeSchema, getD1 } from "@/db/runtime";
...
export async function GET(_request: Request, { params }: { params: Promise<{ trackingCode: string }> }) {
  try {
    ...
    const order = await db.prepare("SELECT tracking_code, status, total_paisa, destination, created_at FROM orders WHERE tracking_code = ?").bind(code).first<{ tracking_code: string; status: string; total_paisa: number; destination: string; created_at: string }>();
