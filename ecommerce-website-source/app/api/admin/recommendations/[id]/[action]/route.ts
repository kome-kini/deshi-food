...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; action: string }> }) {
  try {
    ...
    const db = getD1(); await ensureRuntimeSchema(db); const recommendation = await db.prepare("SELECT id, status FROM recommendations WHERE id = ?").bind(id).first<{ id: string; status: string }>();
    if (!recommendation) return Response.json({ error: "Recommendation not found" }, { status: 404 });
    if (recommendation.status !== "open") return Response.json({ id, status: recommendation.status, error: "Recommendation already decided" }, { status: 409 });
